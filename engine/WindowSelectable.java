...
public class WindowSelectable extends Window {
    ...
    public WindowSelectable(int x, int y, int width, int height) throws SlickException {
    ...
    public void update(InputProvider input, int delta) {
        if (itemMax > 0) {
            ...
            if(input.isCommandControlPressed(SceneBase.pUp)){
                cursorPageUp();
            }
            if(input.isCommandControlPressed(SceneBase.pDown)){
                cursorPageDown();
            }
            if (index != lastIndex) {
                Sounds.cursor.play();
